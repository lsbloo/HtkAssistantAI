...
from core.utils.design.observer.observer import (
    ClientObserver as ConfigurationInterfaceObserver,
)
...
class MainFrame(Subject):
    def _create_root(self):
        ...
        self.root.iconbitmap(
            HtkOsEnvironment.get_absolute_path_for_resource(resource_file="no-face.ico")
        )  # Set the window icon
    ...
    def _create_circular_widget(self):
        ...
        self.circle_canvas = tk.Canvas(
            self.root, width=100, height=100, bg="#252526", highlightthickness=0
        )
        self.circle_canvas.place(
            x=680, y=1
        )  # Position the widget in the top-right corner

        # Draw a circle
        self.circle_canvas.create_oval(
            5, 5, 95, 95, outline="", fill="#252526"
        )  # Circle background

        # Load and resize the image
        try:
            image_path = Image.open(
                HtkOsEnvironment.getImageProfileInEnvironment()
            ...
            # Apply the mask to the image
            self.profile_image = self.profile_image.convert(
                "RGBA"
            ...
            self.circle_canvas.create_image(
                50, 50, image=self.profile_photo, anchor="center"
            )
    ...
    def _create_options_area(self, input_options):
        ...
        self.model_input_option_dropdown = ttk.Combobox(
            self.root,
            textvariable=self.model_input_option_var,
            state="readonly",
            font=("Arial", 12),
        )
        ...
        for role in self.configurations_role:
            if role.name == option.label:
                ...
                if role.isChecked and self.actions.get(option.id) is not None:
                    ...
                    role.isChecked == False and self.actions.get(option.id) is not None
                ):
    ...
    def _submit_on_recon(self, response):
        ...
        selected_option = (
            ...
            if hasattr(self, "model_input_option_var")
            else None
        )
