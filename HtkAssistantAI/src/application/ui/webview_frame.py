...
class WebViewFrame(Subject):
    def __init__(self, app_root, title="HTK Assistant AI - WebMode"):
        ...
        self._app.iconbitmap(
            HtkOsEnvironment.get_absolute_path_for_resource(resource_file="no-face.ico")
        )  # Set the window icon
        self._web_frame = HtmlFrame(
            self._app, horizontal_scrollbar="auto", vertical_scrollbar="auto"
        )
        self._web_frame.load_file(
            HtkOsEnvironment.get_absolute_path_for_resource_page_html(
                "webview_page.html"
            )
        )
