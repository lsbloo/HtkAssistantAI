...
class HtkAudioPlayer(Subject):
            ...
            with sr.Microphone() as Font:
                while not self._stop_recon_event.is_set():
                    self._rec.adjust_for_ambient_noise(
                        Font, duration=2
                    )  # Ajuste para ruído ambiente (consigo melhorar isso?)
                    input_audio = self._rec.listen(Font)
                    try:
                        output_in_text = self._rec.recognize_google(
                            input_audio, language="pt-BR"
                        )
                        ...
                        self._logger.log(
                            "Audio recognition failed: UnknownValueError - could not understand audio."
                        )
                    except sr.RequestError:
                        print("HTK Assistant AI - service connection error.")
                        self._logger.log(
                            "Audio recognition failed: RequestError - service connection error."
                        )
    ...
    def initializeRecon(self):
        ...
        self._threadManager.createThreadAndInitialize(
            threadName=self._threadName, target=self.initRecon
        )
