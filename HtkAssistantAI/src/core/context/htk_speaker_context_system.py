...
class HtkSpeakerContextSystem:
    ...
    def initialize_system_audio_context(self, key):
        ...
        self._speaker.play_audio(output, onMixerBusy=lambda busy: self._logger.log(f"Speaker System Audio Context - Mixer Busy: {busy}"))
         
    def speak_response_system(self, message):
        self._speaker.play_audio(message, onMixerBusy=lambda busy: self._logger.log(f"Speaker System Audio Context - Mixer Busy: {busy}"))
