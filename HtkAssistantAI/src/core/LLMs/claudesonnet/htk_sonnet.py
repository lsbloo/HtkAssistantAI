...
class HtkSonnetClient(HtkClientBase):
    def __init__(self, api_key=None):
        ...
        if not api_key:
            raise ValueError(
                "API key for Claude Sonnet is not set in the environment configuration."
            )
...
class HtkSonnetInitializer:
    ...
    def getInstanceSonnet(self):
        if self._instance_groq is None:
            self._instance_groq = HtkSonnetClient(
                environments_config.get("HTK_ASSISTANT_API_KEY_LLM_ANTHROPIC")
            )
