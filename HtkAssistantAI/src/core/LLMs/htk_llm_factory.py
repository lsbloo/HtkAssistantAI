...
def execHtkAssistantModel(response, client: HtkClientBase, callback=None):
    ...
    if option == 'simple_chat':
        if response['is_recon_enabled'] == True:
            response = client.chat(message=response['input_from_recon'], additionalContext=response['option_persona_context_choice'])
            callback({"response": response, "is_recon_enabled": True})
        else:
            response = client.chat(message=response['user_input'], additionalContext=response['option_persona_context_choice'])
            callback({"response": response, "is_recon_enabled": False})
    elif option == 'chat_with_roles':
        if response['is_recon_enabled'] == True:
            response = client.chat_with_roles(message=response['input_from_recon'], role=RoleType.fromType(response['option_role_choice']), additionalContext=response['option_persona_context_choice'])
            callback({"response": response, "is_recon_enabled": True})
        else:
            response = client.chat_with_roles(message=response['user_input'], role=RoleType.fromType(response['option_role_choice']), additionalContext=response['option_persona_context_choice'])
            callback({"response": response, "is_recon_enabled": False})   